...
/*   Updated: 2015/02/02 18:40:20 by tlesven          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			h_draw(t_image *img, t_line *line)
{
	int		i;
	int		cumul;
	int		lvl;
	...
	cumul = line->dx / 2;
	lvl = line->next_lvl;
	pixel_on_image(img, line->x, line->y, color_lvl(line->z));
	while (i <= line->dx)
	{
		line->x += line->xinc;
		cumul += line->dy;
...
static void			v_draw(t_image *img, t_line *line)
{
	int		i;
	int		cumul;
	int		lvl;
...
void				line_draw_img(t_image *img, t_point p1, t_point p2)
{
	t_line	*line;
